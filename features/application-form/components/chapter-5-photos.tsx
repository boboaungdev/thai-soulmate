...
import { ImageCropDialog } from "@/features/members"
...
export function Chapter5Photos({
  ...
  const handleCropComplete = async (file: File, previewUrl: string) => {
    ...
    try {
      ...
      formDataUpload.append("email", data.email || "applicant")
      formDataUpload.append("type", slot)
      formDataUpload.append("path", "applications/photos")

      const res = await uploadFileAction(formDataUpload)

      if (res.success && res.url) {
        // 3. Save permanent Cloudflare R2 URL into form data
        const r2Updates: Partial<ApplicationFormData> = {}
        if (slot === "headshot") r2Updates.headshotUrl = res.url
        else if (slot === "fullLength") r2Updates.fullLengthUrl = res.url
        else if (slot === "casualLifestyle")
          r2Updates.casualLifestyleUrl = res.url
        onChange(r2Updates)
