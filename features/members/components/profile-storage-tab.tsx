...
import {
  getProfileStorageAction,
  createProfileFolderAction,
  renameProfileFolderAction,
  deleteProfileFolderAction,
  uploadProfileFileAction,
  renameProfileFileAction,
...
export function ProfileStorageTab({ profileId }: { profileId: string }) {
  ...
  const handleCreateFolder = async (folderName?: string) => {
    ...
    try {
      ...
      const data = await createProfileFolderAction(profileId, nameToUse.trim())
  ...
  const handleRenameFolder = async () => {
    ...
    try {
      ...
      const data = await renameProfileFolderAction(
        profileId,
        renameFolderItem.id,
  ...
  const handleRenameFile = async () => {
    ...
    try {
      ...
      const data = await renameProfileFileAction(
        profileId,
        renameFileItem.id,
        renameFileName.trim()
      )
  ...
  const handleDeleteFolder = async () => {
    ...
    try {
      ...
      const data = await deleteProfileFolderAction(
        profileId,
        deleteFolderItem.id,
        false
      )
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        const res = await uploadProfileFileAction(formData)
  ...
  const handleDeleteFile = async () => {
    ...
    try {
      ...
      const res = await deleteProfileFileAction(profileId, deleteFileItem.id)
