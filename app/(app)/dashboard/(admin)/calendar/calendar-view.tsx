...
function itemsOnDay(items: CalendarItem[], date: Date, visibleKinds: CalendarKind[]) {
  return items
    .filter((item) => visibleKinds.includes(item.kind))
      ...
      {items.map((item) => (
        ...
        >
          <div className="mb-1 flex items-center gap-2">
            <span className={cn("size-2 rounded-full", kindStyle[item.kind].dot)} />
...
export function CalendarView() {
  ...
  const counts = useMemo(() => {
    return calendarKinds.map((kind) => ({
      ...kind,
      count: mockCalendarItems.filter((item) => item.kind === kind.value).length,
    }))
  }, [])
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <Card className="overflow-hidden py-0">
        ...
        <CardContent className="p-0">
          <div
            className="calendar-shell h-[min(78vh,840px)] min-h-[560px] p-3 sm:p-5"
      ...
      <div className="flex flex-col gap-6">
        <Card>
          <CardHeader className="px-6">
            ...
            <CardDescription>
              Each type stays on its own color so the team can scan the board quickly.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="flex-1">
          <CardHeader className="px-6">
            ...
            <CardDescription>
              Open one list at a time. If tomorrow is a weekend, Monday is shown instead.
            </CardDescription>
          </CardHeader>
          <CardContent className="px-6">
            <Accordion type="single" collapsible defaultValue="today" className="gap-3">
              <AccordionItem
                value="today"
          ...
          {selected ? (
            <>
              <DialogHeader>
                ...
                <DialogDescription>{formatItemWhen(selected)}</DialogDescription>
              </DialogHeader>
              <div className="space-y-3 text-sm">
                <p>{selected.description}</p>
                {selected.person ? (
                  <p className="text-muted-foreground">Contact: {selected.person}</p>
                ) : null}
                {selected.location ? (
                  <p className="text-muted-foreground">Location: {selected.location}</p>
                ) : null}
              </div>
            </>
          ) : null}
