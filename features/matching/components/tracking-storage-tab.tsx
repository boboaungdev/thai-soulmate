...
import {
  getTrackingStorageAction,
  createTrackingFolderAction,
  renameTrackingFolderAction,
  deleteTrackingFolderAction,
  uploadTrackingFileAction,
  renameTrackingFileAction,
  deleteTrackingFileAction,
...
export function TrackingStorageTab({ trackingId }: { trackingId: string }) {
  ...
  const fetchStorageData = useCallback(async () => {
    try {
      const res = await getTrackingStorageAction(trackingId)
      if (!res.success) throw new Error(res.error || "Failed to fetch storage")
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      try {
        const res = await getTrackingStorageAction(trackingId)
        if (!res.success)
          throw new Error(res.error || "Failed to fetch storage")
        if (isMounted) {
          setFolders(res.folders || [])
          setFiles(res.files || [])
          setTotalSize(res.totalSize || 0)
  ...
  const handleCreateFolder = async (folderName?: string) => {
    ...
    try {
      ...
      const res = await createTrackingFolderAction(
        trackingId,
        nameToUse.trim(),
        user?.id
      )
  ...
  const handleRenameFolder = async () => {
    ...
    try {
      ...
      const res = await renameTrackingFolderAction(
        trackingId,
        renameFolderItem.id,
  ...
  const handleDeleteFolder = async (deleteFiles = false) => {
    ...
    try {
      ...
      const res = await deleteTrackingFolderAction(
        trackingId,
        deleteFolderItem.id,
        deleteFiles
      )
  ...
  const handleFileUpload = async (
    ...
    for (let i = 0; i < uploadedFiles.length; i++) {
      ...
      try {
        const res = await uploadTrackingFileAction(trackingId, formData)
  ...
  const handleRenameFile = async () => {
    ...
    try {
      ...
      const res = await renameTrackingFileAction(
        trackingId,
        renameFileItem.id,
        renameFileName.trim()
      )
      ...
      if (!res.success) {
        throw new Error(res.error || "Failed to rename file.")
      }
  ...
  const handleDeleteFile = async () => {
    ...
    try {
      ...
      const res = await deleteTrackingFileAction(trackingId, deleteFileItem.id)
