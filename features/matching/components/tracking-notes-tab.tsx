...
import {
  ...
  updateTrackingNoteAction,
  deleteTrackingNoteAction,
...
export function TrackingNotesTab({
  ...
  const handleAddNote = async () => {
    ...
    try {
      const result = await addTrackingNoteAction(trackingId, {
        message,
        userId: user.id,
        type: selectedType as TrackingNoteType,
      })

      if (result.success && result.note) {
        const updated = [result.note as TrackingNoteWithUser, ...notes]
        setNotes(updated)
        setMessage("")
        ...
        toast.error(!result.success ? result.error : "Failed to add note.")
        toast.error(!result.success ? result.error : "Failed to add note.")
      }
    } catch {
  ...
  const handleUpdateNote = async () => {
    ...
    setIsSubmitting(true)
    try {
      const result = await updateTrackingNoteAction(
        trackingId,
        editingNote.id,
        {
          message: editedMessage,
          type: editedType as TrackingNoteType,
        }
      )

      if (result.success && result.note) {
        const updated = notes.map((note) =>
          note.id === editingNote.id ? (result.note as TrackingNoteWithUser) : note
        )
        setNotes(updated)
        ...
        toast.error(!result.success ? result.error : "Failed to update note.")
        toast.error(!result.success ? result.error : "Failed to update note.")
      }
    } catch {
  ...
  const handleDeleteNote = async () => {
    ...
    try {
      const result = await deleteTrackingNoteAction(trackingId, noteToDelete.id)
